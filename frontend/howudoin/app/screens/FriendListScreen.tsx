...
const FriendListScreen = () => {
  const [friends, setFriends] = useState<string[]>([]); // 'string' tipi kullanıyoruz çünkü arkadaş listesi bir dizi string
  ...
  const fetchFriends = async () => {
    try {
      ...
      // API yanıtını kontrol et ve setFriends dizisini güncelle
      console.log('Friends data from API:', response.data);
      setFriends(response.data); // ['anıl', 'aliabi'] gibi bir diziyi döner
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      {friends.length > 0 ? (
        <FlatList
          data={friends}
          keyExtractor={(item, index) => item.toString()} // keyExtractor'ı kontrol edin
          renderItem={({ item }) => (
            <TouchableOpacity style={styles.friendItem} onPress={() => handleSelectFriend(item)}>
              <Text style={styles.friendName}>{item}</Text> {/* item burada bir string */}
            </TouchableOpacity>
          )}
        />
      ) : (
