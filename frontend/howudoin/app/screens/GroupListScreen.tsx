...
const GroupListScreen = ({ navigation }: { navigation: any }) => {
  ...
  const fetchGroups = async () => {
    try {
      ...
      Alert.alert('Error', 'Gruplar yüklenemedi. Lütfen tekrar deneyin.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGroupDetails = () => {
    if (selectedGroup) {
      ...
      navigation.navigate('GroupDetails', { groupId: selectedGroup.id }); // ID gönderiliyor
    }
  };
    ...
    <View style={styles.container}>
      ...
      {groups.length > 0 ? (
        <FlatList
          ...
          keyExtractor={(item) => item.id} // ID bir String olarak kullanılıyor
        />
      ) : (
