...
const FriendRequestScreen = () => {
  ...
  const sendFriendRequest = async () => {
    ...
    try {
      // JWT token'i AsyncStorage'dan al
      ...
      const response = await axios.post(
        ...
        { receiverName }, // Gönderilecek veriler
        {
          headers: {
            Authorization: `Bearer ${token}`, // JWT token'i header'a ekle
          },
        }
      );
      ...
      if (error.response && error.response.data) {
        Alert.alert('Error', error.response.data); // Sunucudan gelen hata mesajı
      } else {
